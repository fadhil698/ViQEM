...
class CalculateEnergy:
    ...
    def calculate_zne_energy(
        ...
        hamiltonian,
        theta: np.ndarray,
        noise_factors: List[int] = [1, 3], # Update: hanya 1 dan 3
        do_extrapolate: bool = True,
    ):
        """
        Hitung ZNE dengan Digital Gate Folding pada gate 2-qubit.
        
        Mekanisme:
        - Iterasi setiap gate dalam ansatz.
        - Jika gate 2-qubit: amplifikasi menjadi G -> G (G_dag G)^n.
        - Jika gate 1-qubit: biarkan seperti semula.
        - Ekstrapolasi linear ke noise=0.
        """
        theta_arr = np.asarray(theta, dtype=float)
        ...
        for factor in noise_factors:
            try:
                ...
                # --- 1) Digital Gate Folding Khusus 2-Qubit ---
                folded_ansatz = QuantumCircuit(*ansatz.qregs, *ansatz.cregs)
                
                if factor == 1:
                    folded_ansatz = ansatz.copy()
                else:
                    n_pairs = (factor - 1) // 2
                    
                    # Iterasi setiap instruksi (gate) di dalam sirkuit
                    for inst in ansatz.data:
                        op = inst.operation
                        qubits = inst.qubits
                        clbits = inst.clbits
                        
                        # Cek apakah ini gate 2-qubit (dan bukan barrier/measure)
                        if len(qubits) == 2 and op.name not in ['barrier', 'measure']:
                            # Masukkan gate asli (G)
                            folded_ansatz.append(op, qubits, clbits)
                            
                            # Tambahkan pasangan G_dagger dan G sebanyak n_pairs
                            op_inv = op.inverse()
                            for _ in range(n_pairs):
                                folded_ansatz.append(op_inv, qubits, clbits)
                                folded_ansatz.append(op, qubits, clbits)
                        else:
                            # Gate 1-qubit atau operasi lain dibiarkan normal
                            folded_ansatz.append(op, qubits, clbits)

                # --- 2) Jalankan estimator ---
        ...
        # Ekstrapolasi Linear: y = mx + c. Kita cari c (nilai y saat x = 0)
        # polyfit dengan deg=1 pada 2 titik akan menghasilkan garis lurus yang eksak
